...
class CrossMamba(nn.Module):
    ...
    def forward(self,ms,pan):
        ...
        batch, channel, height, width = global_f.shape
        global_f = global_f.reshape(batch, height * width, channel)
        B, HW, C = global_f.shape

        ms = global_f.transpose(1, 2).view(B, C, height, width)
        ms =  (self.dwconv(ms)+ms).flatten(2).transpose(1, 2)
        ms = ms.transpose(1,2).view(B ,C , height, width)

        half_dim = C // 2

        rgb_fea_out = ms[:, x_dim2:, :, :]
        radar_fea_out = ms[:, :x_dim2,:,:]
...
class TokenSwapMamba(nn.Module):
    def __init__(self, dim):
        ...
        self.msencoder = Mamba(dim,bimamba_type=None).cuda()
        self.panencoder = Mamba(dim,bimamba_type=None).cuda()
        self.norm1 = LayerNorm(dim,'with_bias')
        self.norm2 = LayerNorm(dim,'with_bias')
    # def forward(self, ms,pan
