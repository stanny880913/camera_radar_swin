...
def _fill_trainval_infos_all_cams(nusc, train_scenes, val_scenes, mini=False):
    ...
    if mini:
        num_train_sample = num_val_sample = 150
        ct_train = ct_val = 0
        random.seed(10)
    ...
    for sample_idx in mmcv.track_iter_progress(sample_indices):
        ...
        if (sample["scene_token"] in train_scenes) or (
            sample["scene_token"] in val_scenes
        ):
            if mini:
                if (ct_train > num_train_sample) and (ct_val > num_val_sample):
                    break
...
def create_nuscenes_infos_all_cams(
    root_path, out_dir, info_prefix, version="v1.0-trainval"
):
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos_all_cams(
        nusc, train_scenes, val_scenes
    )

    train_nusc_infos_mini, val_nusc_infos_mini = _fill_trainval_infos_all_cams(
        nusc, train_scenes, val_scenes, mini=True
    )
    ...
    print(
        "mini train sample: {}, mini val sample: {}".format(
            len(train_nusc_infos_mini), len(val_nusc_infos_mini)
        )
    )
    data["infos"] = train_nusc_infos_mini
    info_path_mini = osp.join(out_dir, "{}_infos_train_mini.pkl".format(info_prefix))
    mmcv.dump(data, info_path_mini)
    data["infos"] = val_nusc_infos_mini
    info_val_path_mini = osp.join(out_dir, "{}_infos_val_mini.pkl".format(info_prefix))
    mmcv.dump(data, info_val_path_mini)
...
def nuscenes_data_prep(root_path, info_prefix, version, out_dir):
    ...
    info_train_mini_path = osp.join(out_dir, f"{info_prefix}_infos_train_mini.pkl")
    info_val_mini_path = osp.join(out_dir, f"{info_prefix}_infos_val_mini.pkl")
    export_2d_annotation(root_path, info_train_mini_path, version=version)
    export_2d_annotation(root_path, info_val_mini_path, version=version)
